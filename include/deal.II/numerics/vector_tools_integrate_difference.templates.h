...
namespace VectorTools
{
  namespace internal
  {
    ...
    struct IDScratchData
    {
      ...
      Vector<Number>                           exemplar;
      std::vector<Tensor<1, spacedim, Number>> exemplar_vector;
    };
      ...
      if (this->n_components != n_components)
        {
          ...
          exemplar.reinit(n_components);
          exemplar_vector.resize(n_components);
        }

      function_values.resize(n_q_points, exemplar);
      function_grads.resize(n_q_points, exemplar_vector);

      weight_values.resize(n_q_points);
      weight_vectors.resize(n_q_points, weights_exemplar);

      psi_values.resize(n_q_points, exemplar);
      psi_grads.resize(n_q_points, exemplar_vector);
      psi_scalar.resize(n_q_points);

      tmp_values.resize(n_q_points);
      tmp_vector_values.resize(n_q_points, exemplar);
      tmp_gradients.resize(n_q_points);
      tmp_vector_gradients.resize(n_q_points, exemplar_vector);
    }
