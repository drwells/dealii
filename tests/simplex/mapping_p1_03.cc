...
test()
{
  ...
  FESubfaceValues<dim, spacedim> fe_values(mapping, fe, quadrature, flags);
  FESubfaceValues<dim, spacedim> fe_values_2(mapping_fe, fe, quadrature, flags);

  // Same as the other tests: bypass the check for whether or not the current
  // face actually has children by using a DoFHandler instead of a Triangulation
...
main()
{
  ...
  test<2>();
  test<2, 3>();
  // At the present time we do not support tetrahedral subfaces
  // test<3>();
