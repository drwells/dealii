...
namespace GridTools
{
  ...
  /**
   * Delete any SubCellData which erroneously labels an internal face as a
   * boundary face.
   *
   * A possible issue with delete_duplicated_vertices() is that, if a boundary
   * vertex is deleted, then what was previously a boundary face will now be an
   * internal face. For example, considea a mesh comprised of two spatially (but
   * not topologically) adjacent quadrilaterals with eight vertices (two of
   * which are duplicated). This mesh mesh has no internal faces. If we delete
   * duplicated vertices, then what was a boundary face is now an internal face
   * and setting a boundary_id on that face will result in an exception in
   * Triangulation::create_triangulation(). This situation can occur when
   * loading in data from an external program containing both useful boundary_id
   * values and duplicated vertices.
   */
  template <int dim, int spacedim>
  void
